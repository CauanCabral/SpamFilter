...
#define VERSION     "version 1.8 (2008.10.30)         " \
...
{                               /* --- main function */
  ...
  int    mode     = 0;          /* mode for attribute map */
  ...
  attmap = am_create(attset, mode, ind);
  ...
  /* --- read table header --- */
  seps = as_chars(attset, recseps, fldseps, blanks, nullchs, comment);
  ...
  while (i == 0) {              /* record read loop */
    ...
    for (k = 0; k < n; k++) {   /* execute the attribute map */
