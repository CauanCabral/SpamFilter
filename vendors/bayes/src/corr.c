/*----------------------------------------------------------------------
  ...
  History : 1999.04.07 file created from file xmat.c
            1999.04.10 first version completed
            1999.04.11 options -x, -v, and -c added
            1999.04.14 option -k (expected values from known pairs)
            1999.12.01 bug in connection with option -d removed
            2000.11.10 adapted to new module mvnorm
            2002.06.07 LaTeX output option added
            2003.08.16 slight changes in error message output
            2004.04.22 bug in function dblout fixed (log(0))
...
#define VERSION     "version 2.9 (2008.10.30)         " \
...
{                               /* --- add a column to a column set */
  ...
  cset->vals [cset->colcnt++] = MVN_NULL;
...
{                               /* --- set a column value */
  ...
  if (!name) {                  /* if no value name is given, */
    *val = MVN_NULL; return; }  /* the value is null */
  *val = strtod(name, &s);      /* convert the attribute value */
  if (*s || (s == name)) {      /* if it is nominal */
    *val = MVN_NULL; cset->names[colid] = NULL; }              
...
{                               /* --- print correlation coefficients */
  ...
  for (y = 0; y < cset->colcnt; y++) {
    ...
    for (x = y+1; x < cset->colcnt; x++) {
      ...
      if (r < MVN_NULL) { fputs("     ?", out); continue; }
...
{                               /* --- print error message */
  ...
  if (tscan)  ts_delete(tscan);    /* clean up memory */
...
{                               /* --- main function */
  ...
  /* --- create symbol table and table scanner --- */
  ...
  ts_chars(tscan, TS_NULL,    nullchs);
  ...
  do {                          /* read fields of table header */
    ...
    if (d == TS_ERR) error(E_FREAD, fname);
    ...
    if (header != 1) {          /* if to use a normal header */
      ...
      *p = fldcnt-1;            /* note the field number */
      if (cs_add(colset, st_name(p)) != 0)
        error(E_NOMEM);         /* add a column to the column set */
      cs_set(colset, fldcnt-1, (rdbuf[0] == '\0') ? NULL : rdbuf);
